...
describe('Stores Endpoints', function() {
  ...
  before('clean the table', () => db.raw('TRUNCATE sustainability_ratings, sustainability_packagings, sustainability_categories, sustainability_stores RESTART IDENTITY CASCADE'))

  afterEach('cleanup', () => db.raw('TRUNCATE sustainability_stores, sustainability_ratings, sustainability_packagings, sustainability_categories RESTART IDENTITY CASCADE'))
  ...
  describe(`GET /api/stores`, () => {
    ...
    context('Given there are stores in the database', () => {
      ...
      beforeEach('insert stores', () => {
        ...
        .then(() => {
            return db
            .into('sustainability_categories')
  ...
  describe(`POST /api/stores`, () => {
    ...
    it(`creates a store, responding with 201 and the new store`, () => {
      ...
      return supertest(app)
        ...
        .expect(res => {
          ...
          expect(res.headers.location).to.eql(`/api/stores/${newStore.storeid}`)
        })
        .then(res =>
          supertest(app)
    ...
    const requiredFields = ['storename','website','comments','categoriesid','ratingsid','packagingsid']
  ...
  describe(`DELETE /api/stores/:storeid`, () => {
    ...
    context('Given there are stores in the database', () => {
      ...
      it('responds with 204 and removes the store', () => {
        const idToRemove = 2
        const expectedStores = testStores.filter(store => store.storeid !== idToRemove)
        return supertest(app)
